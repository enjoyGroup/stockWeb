...
public class InvoiceCashMaintananceServlet extends EnjoyStandardSvc {
	...
	private void onSave() throws EnjoyException{
		...
		try{
			...
			this.invoiceCashDao.deleteInvoiceCashDetail(invoiceCode, tin);
			for(int i=0;i<invoiceCashDetailList.size();i++){
				...
				if(!bean.getRowStatus().equals(InvoiceCashMaintananceForm.DEL)){
					...
					this.invoiceCashDao.insertInvoiceCashDetail(bean);
					seqDb++;
	...
	private void getProductDetailByName(){
	   ...
	   try{
		   ...
		   if(productmasterBean!=null && !tin.equals("")){
			   ...
			   discount = this.productDetailsDao.getQuanDiscount(productmasterBean.getProductCode(), quantity, invoiceDate, tin, CASH);
	...
	private void getDiscount() {
		...
		try {
			...
			discount = this.productDetailsDao.getQuanDiscount(productCode, quantity, invoiceDate, tin, CASH);
	...
	private void onCancel() throws EnjoyException{
		...
		try{
			...
			if(!invoiceCode.equals("")){
				...
				this.invoiceCashDao.updateInvoiceCashMasterStatus(invoiceCashMasterBean);
	...
	private void getProductDetailByCodeDis(){
	   ...
	   try{
		   ...
		   if(productmasterBeanDb!=null && !tin.equals("")){
			   ...
			   discount = this.productDetailsDao.getQuanDiscount(productmasterBeanDb.getProductCode(), quantity, invoiceDate, tin, CASH);
