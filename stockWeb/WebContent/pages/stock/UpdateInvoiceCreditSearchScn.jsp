...
<jsp:useBean id="updateInvoiceCreditSearchForm" class="th.go.stock.app.enjoy.form.UpdateInvoiceCreditSearchForm" scope="session"/>
...
<head>
	...
	<script>
				...
				try{
					$.ajax({
			            ...
			            success: function(data){
			            	gp_progressBarOff();
			            	window.location.replace('<%=pagesURL%>/UpdateInvoiceCreditSearchScn.jsp');
			            }
			        });
				}catch(err){
					alert("btnSearch :: " + err);
		...
		function lp_sendEditPage(av_invoiceCode){
			var params = "";
			try{
				params = "invoiceCode=" + av_invoiceCode + "&updateCredit=Y";
...
<body>
	<form id="frm">
		<input type="hidden" id="service" 	name="service" value="servlet.UpdateInvoiceCreditSearchServlet" />
										...
										<table class="table sim-panel-result-table" id="tbl_result" border="1" width="100%">
											<thead> 
								               <tr height="26px;">
													...
													<th  style="text-align: center;" width="12%"><B>วันที่ขาย</B></th>
													<th  style="text-align: center;" width="12%"><B>จำนวนเงิน</B></th>
													<th  style="text-align: center;" width="26%"><B>หมายเหต</B></th>
												</tr> 
											</thead>
											<tfoot>
												<tr height="26px;">
													<td colspan="7" align="right">
														...
														<input type="text" id="i_txt_nvt_totalresult" name="i_txt_nvt_totalresult" style="top:0px;left:0px;width: 50px;"  readonly="readonly" value="<%=updateInvoiceCreditSearchForm.getTotalRecord()%>" >
														...
														<input type="text" id="selPage" name="selPage" style="top:0px;left:0px;width: 30px;text-align: right;" maxlength="3" readonly="readonly" value="<%=updateInvoiceCreditSearchForm.getPageNum()%>">
									
														<span class="c_field_label" style="top:-5px;">/</span>
														<span id="i_txt_nvt_total" class="c_field_label" style="top:-5px;" name="i_txt_nvt_total"><%=updateInvoiceCreditSearchForm.getTotalPage()%></span>
														<input type="hidden" id="txtMaxresult" name="txtMaxresult" value="<%=updateInvoiceCreditSearchForm.getTotalPage()%>" >
