...
public class StockMovingHeader extends PdfPageEventHelper {
	
	private JSONObject 	formDataObj;
	private EnjoyItext	enjoyItext;
	
	public StockMovingHeader(JSONObject formDataObj) {
	...
	public void onStartPage(PdfWriter writer, Document document) {
		try{
			document.add(enjoyItext.genHeader(formDataObj, "รายงานเคลื่อนไหว Stock สินค้า"));
			document.add(brLine());
...
//		table.addCell(enjoyItext.setCellWB("รายงานเคลื่อนไหว Stock สินค้า", enjoyItext.getFont10Bold(), 1, Element.ALIGN_CENTER, 0));
//		
...
//	}
	...
	private PdfPTable genHeader1() throws DocumentException, MalformedURLException, IOException {
		...
		rangeReciveDate = enjoyItext.getText(jsonObjectMain, "hisDateFrom") + " - " + enjoyItext.getText(jsonObjectMain, "hisDateTo");
